from arctic3d.modules.interface_matrix import check_residues_coverage, compute_scalar_product, filter_interfaces, get_coupling_matrix, interface_matrix
...
def test_check_residues_coverage():
    ...
    pdb_resids = [1,2,3,4]
    cov_one, filtered_int_one = check_residues_coverage(interface_one,
                                                        pdb_resids)
    assert cov_one == 1.0
    assert filtered_int_one == interface_one

    cov_two, filtered_int_two = check_residues_coverage(interface_two,
                                                        pdb_resids)
...
def test_get_coupling_matrix(example_mdu):
    ...
    int_resids = [1,2,3]
    expected_jij = np.array([[1.0, 0.358133, 0.031553],
...
def test_compute_scalar_product():
    ...
    jij = np.array([[1.0, 0.358133, 0.031553],
                    [0.358133, 1.0, 0.366509],
                    [0.031553, 0.366509, 1.0]])
    interface_one = [0,1,2]
...
def test_filter_interfaces(example_mdu, example_interface_dict):
    ...
    expected_filter_dict = {"int_1" : [1, 2],
                            "int_2" : [1, 2, 4]
                            }
